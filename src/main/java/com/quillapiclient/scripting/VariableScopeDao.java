...
public final class VariableScopeDao {
    ...
    public static void persistEnvironment(int environmentId, Map<String, String> vars) {
        persistKeyValues(
            "environment_values",
            "environment_id",
            environmentId,
    ...
    public static void persistCollection(int collectionId, Map<String, String> vars) {
        persistKeyValues(
            "variables",
            "collection_id",
            collectionId,
    ...
    private static Map<String, String> loadKeyValues(String sql, int foreignId) {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                map.put(rs.getString("variable_key"), rs.getString("variable_value"));
            }
        } catch (SQLException e) {
            System.err.println("VariableScopeDao load error: " + e.getMessage());
        }
        return map;
    }

    private static void persistKeyValues(String table, String foreignColumn,
                                         int foreignId, Map<String, String> vars,
                                         int sortOrderStart) {
        Connection conn = LiteConnection.getConnection();
        try {
            ...
            String delSql = "DELETE FROM " + table + " WHERE " + foreignColumn + " = ?";
            ...
            // insert fresh batch
            if (!vars.isEmpty()) {
                String insSql = "INSERT INTO " + table + " (" + foreignColumn +
            ...
            System.err.println("VariableScopeDao persist error: " + e.getMessage());
        } finally {
