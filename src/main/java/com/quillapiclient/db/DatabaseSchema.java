...
public class DatabaseSchema {
    ...
    private static void createIndexes(Statement stmt) throws SQLException {
        // Collection lookups
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_collections_postman_id ON collections(postman_id)"
        );
        stmt.execute(
        ...
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_requests_url_raw ON requests(url_raw)"
        );
        // Full-text search on URL (SQLite FTS5 would be better, but this works)
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_requests_url_search ON requests(url_raw COLLATE NOCASE)"
        );
        ...
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_headers_key ON headers(header_key)"
        );
        ...
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_query_params_key ON query_params(param_key)"
        );

        // Variable lookups
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_variables_collection_id ON variables(collection_id)"
        );
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_variables_item_id ON variables(item_id)"
        );
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_variables_key ON variables(variable_key)"
        );
        ...
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_response_headers_response_id ON response_headers(response_id)"
        );
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_response_headers_key ON response_headers(header_key)"
        );

        // Environment lookups
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_environments_postman_id ON environments(postman_id)"
        );
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_environments_name ON environments(name)"
        );
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_environment_values_env_id ON environment_values(environment_id)"
        );
        stmt.execute(
            "CREATE INDEX IF NOT EXISTS idx_environment_values_key ON environment_values(variable_key)"
        );
    }
    ...
    public static void migrateEventsTable() {
        Connection conn = LiteConnection.getConnection();
        try (Statement stmt = conn.createStatement()) {
            java.sql.ResultSet rs = stmt.executeQuery("PRAGMA table_info(events)");
            boolean needsMigration = false;
            while (rs.next()) {
                if ("collection_id".equals(rs.getString("name")) && rs.getInt("notnull") == 0) {
                    needsMigration = true;
                    break;
                }
            }
            ...
            stmt.execute("CREATE TABLE events_new (" +
                "id INTEGER PRIMARY KEY AUTOINCREMENT, " +
                "collection_id INTEGER NOT NULL, " +
                "item_id INTEGER, " +
                "event_type TEXT NOT NULL CHECK(event_type IN ('prerequest','test')), " +
                "script_type TEXT, script_exec TEXT, full_event_json TEXT, " +
                "FOREIGN KEY (collection_id) REFERENCES collections(id) ON DELETE CASCADE, " +
                "FOREIGN KEY (item_id) REFERENCES items(id) ON DELETE CASCADE)");
            stmt.execute("INSERT INTO events_new SELECT id, collection_id, item_id, " +
                "event_type, script_type, script_exec, full_event_json FROM events");
            stmt.execute("DROP TABLE events");
            stmt.execute("ALTER TABLE events_new RENAME TO events");
            ...
            stmt.execute("COMMIT");
            stmt.execute("PRAGMA foreign_keys = ON");
            System.out.println("Migrated events table schema");
        } catch (SQLException e) {
            try { conn.createStatement().execute("ROLLBACK"); } catch (SQLException ignored) {}
            System.err.println("Events migration failed: " + e.getMessage());
        }
    }
